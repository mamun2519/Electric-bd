...
const ManageOrder = () => {
      ...
      useEffect(() => {
            fetch('https://electic-bd-server-git-main-mamun2232.vercel.app/manageOrder', {
                  method: "GET",
                  headers: {
      ...
      const shippedHendeler = (order) => {
            ...
            if (order?.paid) {
                  ...
                  console.log('cllick');
                  fetch(`https://electic-bd-server-git-main-mamun2232.vercel.app/shipped/${order?._id}`, {
                        method: "PATCH",
                        headers: {
                              'Content-type': 'application/json',
                              'authorization': `Bearer ${localStorage.getItem('accessToken')}`
      ...
      const orderDeleteHendeler = (id) => {
            fetch(`https://electic-bd-server-git-main-mamun2232.vercel.app/booking/${id}`, {
                  method: "DELETE",
                  headers: {
                        'Content-type': 'application/json',
                        'authorization': `Bearer ${localStorage.getItem('accessToken')}`
